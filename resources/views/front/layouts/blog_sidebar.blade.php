...
<div class="category-widget widget">
    ...
    <ul class="category-list">
        @foreach($category as $item)
            <li><a href="{{route('blog.category',['category'=>$item->slug])}}">{{$item->name}}</a></li>
        @endforeach
    </ul>
</div>
    ...
    <ul class="popular-list">
        ...
        <li>
            ...
            <div class="side-content">
                <h2><a href="{{route('blog.post',['category'=>$popular->getCategory->slug,'post'=>$popular->slug])}}">{{$popular->title}}</a></h2>
